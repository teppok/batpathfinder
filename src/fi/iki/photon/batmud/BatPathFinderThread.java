...
/**
 * The class that implements the main bulk of the A* algorithm, running in a
 * thread.
 *
...
class BatPathFinderThread extends Thread {
    ...
    /**
     * Do the solving and after having finished, call SolvedListener with the
     * resulting path and the original type.
     */
    @Override
    public void run() {
        try {
    ...
    /**
     * Reads the class fields and solves the A* problem. Return value is a space
     * separated string of commands that might have integer values n inbetween,
     * which means that the next command must be repeated n times.
     *
     * @return A string containing a string of commands from start node to end
     * node.
     */
...
//				}
    ...
    /**
     * Adds some of the nodes next to the current node to the open heap. If
     * other conditions are met, make changes to the data structures.
     *
     * @param node
     */
